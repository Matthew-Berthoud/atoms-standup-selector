...
const App: React.FC = () => {
  ...
  const handleConfirmSelection = (team: Team) => {
    setTeams(prev => prev.map(t => 
      t.id === team.id ? { ...t, isCompleted: true } : t
    ));
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold tracking-tight text-white">
                ATOMS <span className="text-brand-gold">Standup of Standups</span>
              </h1>
              <p className="text-xs text-gray-500 font-medium tracking-wider uppercase">Team Picker</p>
            </div>
          </div>
                ...
                <span className="inline-flex h-full w-full cursor-pointer items-center justify-center rounded-full bg-[#1a1a1a] px-12 py-6 text-xl font-bold text-white backdrop-blur-3xl transition-all group-hover:bg-[#262626]">
                  <span className="flex items-center gap-3">
                    {isThinking ? (
                      ...
                      <>
                        <Dices className={remainingCount > 0 ? "text-brand-gold" : "text-gray-500"} />
                        <span className={remainingCount > 0 ? "text-white" : "text-gray-500"}>
